	...
	return (
		<>
			...
			{workouts.length > 0 ? (
								...
								<CardHeader className="px-5 pt-4 flex-col items-start">
									<div className="flex justify-between gap-2 w-full items-center">
										<div className="tracking-tight grow">
											<time>{format(new Date(activity.createdAt), 'dd/MM/yyyy')}</time>
											<span className="text-gray-400"> | </span>
											<span><FormatDuration seconds={activity.duration} /></span>
											<span className="text-gray-400"> | </span>
											<span>{totalWeight} Kg</span>
										</div>
										<ActivityMenu activity={activity} />
									</div>
									<p className="text-sm text-zinc-500 leading-5">{activity.name}</p>
								</CardHeader>
				...
				<p>No workouts have been completed. <Link className="text-primary" href="/workout">Click here to start one</Link>.</p>
			)}
		</>
	);
}
