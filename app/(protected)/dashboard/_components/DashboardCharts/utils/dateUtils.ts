...
export function calculateStartDate(dateRange: '3D' | '1W' | '1M' | '3M' | '1Y') {
    switch (dateRange) {
        case '3D':
...
export const calculateIntervals = (dateRange: string) => {
  const startDate = calculateStartDate(dateRange as '3D' | '1W' | '1M' | '3M' | '1Y');
  let intervals;
  ...
  switch (dateRange) {
    ...
    case '1M':
    case '3M':
      intervals = eachWeekOfInterval({ start: startDate, end: new Date() }, { weekStartsOn: 1 });
      break;
    case '1Y':
...
export const getIntervalStartAndEndDates = (interval: Date, dateRange: string) => {
