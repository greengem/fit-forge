...
export default function RecordsTab({ exerciseId }: { exerciseId: string | undefined }) {
    ...
    const processRecords = (workouts: WorkoutLog[]): { bestByReps: Array<{ weight: number; date: string }>, maxVolume: { volume: number; weight: number; reps: number; date: string }, max1RM: number } => {
        let bestByReps = Array.from({ length: 12 }, () => ({ weight: 0, date: '' }));
    ...
    return (
        <div>
            ...
            <ul className="space-y-3">
                ...
                <li className="flex justify-between">
                    <span>Weight</span>
                    <span>{maxVolume.weight}KG (x{maxVolume.reps})</span>
                </li>
                <li className="flex justify-between">
