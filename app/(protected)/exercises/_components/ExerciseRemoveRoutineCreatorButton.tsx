...
export default function ExerciseRemoveRoutineCreatorButton({ exerciseId }: { exerciseId: string }) {
  const searchParams = useSearchParams();
  const routineId = searchParams.get("id");

  const handleClick = async () => {
    if (routineId) {
      const response = await handleRemoveExerciseFromRoutine({ exerciseId, routineId });
      if (response.success) {
