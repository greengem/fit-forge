...
function toEquipmentType(items: string[]): EquipmentType[] {
    return items.filter((item): item is EquipmentType => Object.values(EquipmentType).includes(item as EquipmentType));
}
...
export default function ProfileEquipment({ equipment }: ProfileEquipmentProps) {
    ...
    const handleSubmit = async () => {
        ...
        const response = await handleUpdateUserEquipment(toEquipmentType(selectedEquipment));
    ...
    return (
        <Card shadow="none" className='shadow-md'>
            ...
            <CardFooter className='px-5'>
                <Button color='primary' onPress={handleSubmit} isLoading={isLoading} startContent={<IconDeviceFloppy />}>Save</Button>
            </CardFooter>
        </Card>
    )
}
