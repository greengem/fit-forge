...
export async function handleAddExerciseToNewRoutine(data: NewRoutineFromExercise) {
    ...
    try {
        ...
        const reps = trackingType === TrackingType.reps ? parseInt(data.reps || '0') : null;
        const duration = trackingType === TrackingType.duration ? parseInt(data.duration || '0') : null;
...
export async function handleAddExerciseToExistingRoutine(data: UpdateRoutineFromExercise) {
    try {
        const { userId } : { userId: string | null } = auth();
        ...
        const order = existingExercises.length > 0 ? existingExercises[0].order + 1 : 1;
